     ...
     def _sqrtm_autograd(self, x):
         ...
         if iterN < 2:
             ...
             for _ in range(iterN - 2):
                 ...
                 Y = Y.bmm(ZY)
                 Z = ZY.bmm(Z)
                 #print(torch.norm(Y[0]*Z[0]-torch.eye(dim, dim, device=x.device).view(1, dim, dim).repeat(batchSize, 1, 1).type(dtype)))
             YZY = 0.5 * Y.bmm(I3 - Z.bmm(Y)) #original version
         y = YZY * torch.sqrt(normA).view(batchSize, 1, 1).expand_as(x) #original version
         return y
     def _triuvec(self, x):
