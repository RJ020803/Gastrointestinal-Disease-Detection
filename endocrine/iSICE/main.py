...
parser.add_argument('--dist-url', default='tcp://127.0.0.1:50', type=str,
...
parser.add_argument('--sparsity', default=0.01, type=float,
                    help='sparsity value (default: 0.01)')
parser.add_argument('--iterations', default=5, type=int,
                    help='solver iterations (default: 5)')
parser.add_argument('--sicelr', default=5.0, type=float,
                    help='solver learning rate (default: 5)')
best_prec1 = 0
...
def main():
    ...
    elif args.representation == 'COV':
        representation = {'function':COV,
                          'iterNum':5,
                          'is_sqrt':True,
                          'is_vec':True,
                          'input_dim':2048,
                          'dimension_reduction':None}
    elif args.representation == 'INVCOV':
        representation = {'function':INVCOV,
                          'iterNum':7,
                          'is_sqrt':True,
                          'is_vec':True,
                          'input_dim':2048,
                          'dimension_reduction':256}
    elif args.representation == 'SICE':
        representation = {'function':SICE,
                          'iterNum':args.iterations,
                          'is_sqrt':True,
                          'is_vec':True,
                          'input_dim':2048,
                          'dimension_reduction':256,
                          'sparsity_val':args.sparsity,
                          'sice_lrate':args.sicelr}
    else:
...
def adjust_learning_rate(optimizer, lr_factor, epoch):
    ...
    for param_group in optimizer.param_groups:
        ...
        print('the learning rate is set to {0:.8f} in {1:} part'.format(param_group['lr'], groups[num_group]))
        num_group += 1
